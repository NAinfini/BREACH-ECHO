...
多人加入使用 **Host Package Lock 自动同步**：Host在正式连接前提供按端分类的精确包锁，Client只为双方必需部分补齐缺失的精确版本/hash，再验证、staging并激活临时Profile；玩家不需要手工修改baseline文件或逐个寻找共享Mods。三类边界由MODRUNTIME-SIDE及其责任合同明确。

自动同步不能绕过安全边界：Data/Graph/受支持Luau Sandbox Script可以走标准一键同步；初始版本完全拒绝社区Managed Assembly、Native DLL、可执行文件和动态库，不能因加入Lobby静默执行任意本地代码。完整边界见[Steam Workshop Mod Runtime](../../技术/模组运行时与脚本沙箱.md)。
...
加入状态机、下载集合和回滚仅由[安全与同步合同](../../技术/模组安全包固定与自动同步.md)拥有；玩家操作由[管理器](../../玩法/游戏内模组管理器.md)拥有。

MODRUNTIME-SIDE · 2026-09-06 · CANON：所有者明确要求host_only、client_only、host_and_client三类；加入只下载双方必需包，保留玩家自己的客户端模组。来源：SRC-USER-2026-09-06-MOD-SIDE-SYNC。字段、依赖限制及迁移资格为受托DECIDED，尚未实现。

选择按端生成共享下载集合与个人客户端组合，解决全量克隆房主Profile会安装无关包并覆盖个人体验的问题。淘汰“全部房主模组都必须匹配”，因为它违背本次明确要求；淘汰“只过滤顶层标签”，因为传递依赖和同一Workshop item仍可能带入单端包；淘汰“Gameplay脚本只在主机执行就算主机专用”，因为新武器/地图仍可能要求客户端定义和资产。

约束是共享内容必须完整、个人包不能覆盖共享玩法，且主机迁移另需完整权威闭包；因此普通加入资格与接任主机资格分开。影响PackageManifest、Package Lock、Join Profile、下载队列、握手及选主；责任分别见[安全同步](../../技术/模组安全包固定与自动同步.md)、[迁移](../../技术/主机迁移与恢复协议.md)及[A-MODS验收](../../制作/验收矩阵与证据要求.md)。若单端声明无法通过内容/依赖验证，拒绝该包并要求作者修正发布；若无合格换主成员，明确显示恢复限制，不以自动分发单端包绕过所有者要求。下一实现证明为三类混合包的加入、失败回滚和换主缺包测试。
...
以下路径均相对于仓库中的`game/`工程根，不是仓库根；这是唯一工程目录定义，不代表当前已创建。只在责任里程碑有真实消费者时建立相应目录：
...
- `Assets/Breach`：Unity表现、场景、美术与音频；第三方与生成资产按许可政策另行隔离，不因放进Assets便可进入公共Git；
