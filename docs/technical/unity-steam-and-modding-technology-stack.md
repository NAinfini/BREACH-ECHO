...
depends_on: ["../governance/decisions/unity-engine-and-rendering.md", "../research/technical-evidence-2026-09-05.md"]
---
...
| Mod 脚本 | Luau Core Compiler + VM；Authority-only Gameplay Script；白名单 Capability API | 不使用 Lute、MoonSharp、动态 C#、任意 WASM 或社区 DLL；发布须过 IL2CPP/安全/性能 Spike |
...
FishNet当前没有内建Host Migration；它的ColliderRollback文档也将该组件列为Pro功能。首发选择自有、范围受限的命中体历史，而不是假装买到了付费回滚插件。先证明命中和迁移，再扩大对象量。技术 Spike 失败时在相同合同下更换适配实现并更新对应描述性决策记录；不因一个例子连上网就宣布全部满足。

## 服务器回溯命中

STACK-003 · DECIDED；时间值TEST，归测试参数。

仅对经验证的 hitscan 请求进行有限历史命中体查询。Authority 保存稳定 EntityID/PartID、碰撞代理形状、位置、姿态及门/可破坏遮挡的相关历史；客户端发送输入序号和经服务器时钟估计限制的射击时间，不发送可信的命中结果。最大回看初值为 **150 ms TEST**，不是永久值或 Ping 上限；未来时间、过旧请求、非法射速/弹药/姿态均拒绝或按明确当前时刻规则处理并计数。
