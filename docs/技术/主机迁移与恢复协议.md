...
Authority 首轮每 10 秒制作一个 tick 边界的自包含 Recovery Baseline，并以 2 Hz 发送可验证 Delta；加入、完成、挂起和 graceful transfer 立即触发边界 Baseline。接收端必须能用 Baseline + 连续 Delta 重建独立完整状态，缺片或摘要不符就请求新 Baseline。两名及以上真人在线时至少一名非 Host 的合格存储备份持有闭合恢复链；有三名非 Host 时优先复制到两名。存储资格不要求安装主机专用模组；选主还须满足MIG-MOD-SIDE的执行资格。候选未持有闭合链就不可选主。
...
MIG-MOD-SIDE · DECIDED。选主内容条件按[MODSEC-SIDE](模组安全包固定与自动同步.md)检查：候选必须已经在本地拥有并验证原Run的shared_packages与host_packages精确闭包，以及主机模组状态恢复能力。普通客户端可保存有界恢复记录并作存储ACK；仅持有记录不证明能执行主机逻辑。完整包缺失的成员不能登记为可执行恢复候选，也不能计入跨机恢复保障。不得在加入、热备或突然换主时自动下载安装host_only包。

房间准备阶段显示是否有合格换主候选；缺包者仍可作为普通客户端加入。若要保留换主保障，由愿意接任的玩家在自己的管理器中主动准备主机专用包，再重新验证资格。发生迁移而无人具备完整闭包时进入RecoveryBlocked，保持原锁与可恢复材料，不静默停用主机模组。不得用安装了共享包推断所有玩家都能接任主机。
