...
Read the root README and AGENTS when present, then docs/文档总览.md, docs/治理/文档编写与权威规则.md, docs/治理/当前决策登记.md, relevant responsibility documents, and referenced decisions. Inspect the actual repository, not memory alone. Preserve explicit owner decisions. Source snapshots are immutable historical evidence, not an alternative active specification. Future expansion plans and unimplemented candidates are maintained design material, not obsolete discussion to delete.
...
4. Handoff and integrity: one responsibility owner per rule, stable IDs, descriptive Chinese document filenames, relative Markdown links compatible with both Obsidian and GitHub, complete registry, a beginner reading path, implementation order, owner decision queue and automated validation. Keep tool-mandated names such as README.md, AGENTS.md and SKILL.md unchanged.
