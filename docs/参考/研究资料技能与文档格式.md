...
没有唯一强制GDD模板。按受众与决策拆分，系统规则单一责任、内容卡可制作、技术契约可验证、制作Gate可否决。采用短且可追溯的活文档，而不是机械按源45章拆成45份镜像。网上“skill”或提示词不能替代游戏研究/测试；本轮未下载或安装第三方技能。适合当前任务的是设计方法、GDD、主题决策记录、内容卡与实验格式，见[作者规则](../治理/文档编写与权威规则.md)及[模板](../模板/系统规格模板.md)。
...
REF-007 · 2026-09-07 · 一手资料核对与本项目方法建议。没有按销量或页数评选“全球最好GDD”；选择能展示实际设计沟通、关卡生产和可验证规则的公开样本。下表的采用是方法推断，不是BREACH已经取得相同效果。

| 公开样本 | 本次实际读取范围 | 对本项目的具体用途与限制 |
|---|---|---|
| [Splash Damage：Dirty Bomb设计文档发布页](https://www.splashdamage.com/news/the-design-of-dirty-bomb/) | 开发者发布说明；其300多页PDF链接请求超时，未逐页审阅 | 证实真实开发材料包含设计、草图、用户故事等，而非只有设定列表。它是历史汇编，不适合照搬成当前唯一规则库 |
| [Splash Damage：Dirty Bomb关卡制作复盘](https://www.splashdamage.com/news/blocktober-dirty-bomb/) | 关卡设计师关于目标、流程图、二维俯视、灰盒、武器/角色适配及试玩的正文 | 任务合同应连到空间图、行动距离和完整试玩。采用其“目标到空间到验证”的联系，不搬用PvP复活距离、职业或地图数 |
| [Valve：Left 4 Dead的AI系统设计](https://cdn.akamai.steamstatic.com/apps/valve/2009/ai_systems_of_l4d_mike_booth.pdf) | 设计目标、AI职责、动作生命周期、Bot及重玩/人口管理部分文本；不是逐页图像核验 | 学习目标、运行职责、失败原因与调试观察的连接。不能据此引入无限补怪、清除现存敌人或Bot传送；本项目有限来源和持续追杀不变 |
| [Tim Ryan：设计文档结构，第一部分](https://www.gamedeveloper.com/design/the-anatomy-of-a-design-document-part-1-documentation-guidelines-for-the-game-concept-and-proposal) | 原作者关于文档目的、团队读者、概念/提案与后续规格分工的正文 | 用来检查文档是否足以指导不同岗位，而非堆术语。1999年的实践文章不是当前强制标准，也不是要求先写完全部量产内容才能验证核心 |
| [Microsoft：XAG 103，多通道信息](https://learn.microsoft.com/en-us/xbox/accessibility/xbox-accessibility-guidelines/103) | 目标、适用问题、多通道与颜色非唯一信息、验证限制 | 核对危险、状态与交互不能只靠声音、颜色或震动；合法信息精度不增加。指南与模拟过滤器均不等于实际玩家测试或认证 |

本项目继续采用现有四层结构：产品说明承诺；玩法规则说明行为；内容卡给出可搭实例；技术与制作合同说明所有权、失败与验收。已有[系统模板](../模板/系统规格模板.md)和[内容模板](../模板/内容规格模板.md)覆盖这些字段，不再建立一套平行大GDD。当前不足是部分实例和验证证据，而不是缺另一种文件格式；具体结论见[全项目审查](../制作/游戏开发准备度与责任覆盖矩阵.md)。

本次Unity Learn GDD页面及Dirty Bomb完整PDF未能成功读取；Diablo提案未取得可用正文，不将其细节作为本次判断依据。未安装新技能或复制外部游戏文档正文。

## 可访问性与平台

| 来源ID / 等级 | 原始资料 | 对应文档与采用 |
|---|---|---|
| REF-XAG / E1 | [Xbox Accessibility Guidelines](https://learn.microsoft.com/en-us/xbox/accessibility/guidelines) | [UX](../表现与资产/界面信息与可访问性.md)的早期设计guardrail，不宣称法律或认证通过 |
