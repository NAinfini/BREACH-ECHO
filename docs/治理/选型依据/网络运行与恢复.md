...
对局 Authority 仍运行在玩家主机；协调器不模拟敌人、物理、任务或 60 Hz 世界，也不保存完整对局。它解决 Steam Lobby 无法可靠解决的唯一新主、租约、分区与恢复证书问题，并与[在线活动控制面](../../技术/在线活动控制面与版本快照.md)共享认证和服务基础，用于活动目录、幂等奖励/贡献与未来Descent公共进度；这些职责不把它升级为每局官方专服。
...
首轮测试参数是 `10 秒 Baseline + 2 Hz Delta + 即时 Journal`；普通战斗瞬态目标回退不超过 500 ms，已确认 durable transaction 必须零重复、零遗漏。以上都是 TEST，不是宣传 SLA；数据表只在[主机迁移合同](../../技术/主机迁移与恢复协议.md)和[测试配置](../../制作/初始测试参数.md)维护。
