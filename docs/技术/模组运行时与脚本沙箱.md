...
- 标准Package只接收可读Luau源码，不接收作者提供的预编译字节码。包身份按MODSEC-PACKAGE-IDENTITY计算，安装端编译不会改变发布包hash。激活前由项目固定版本的受信Compiler验证并编译源码，字节码仅在本次运行内存中使用，不从Workshop或可改写的磁盘字节码缓存加载。编译器版本、API schema和目标构建纳入运行锁/诊断，不把派生产物回写发布包。
- 每个Package使用独立environment与内存记账；共享标准库只读，不共享可变global。内存类别不是限额：宿主分配器必须拒绝超配额的VM分配，宿主句柄/缓冲区/查询结果也计入所属包。可信宿主代码与不可信脚本不得共用可变对象或高权限脚本环境。
...
MODRUNTIME-COMPILER-BOUNDARY · 2026-09-07受托DECIDED。采用源码本地受控编译，拒绝仅凭hash信任任意字节码；[Luau官方沙箱说明](https://luau.org/sandbox/)明确区分编译器输出与不可信字节码，并说明CPU中断不保证及时打断长时间宿主函数。因而入编译器前限制源码大小、数量与依赖深度；编译本身有时间/内存预算和失败诊断，宿主API必须有界、不可阻塞等待网络/文件或返回无限结果。预算数值仍由Spike确定，不预设任意脚本都安全。若重新编译的开销妨碍加入，先测量与优化；任何持久字节码缓存必须另补完整性与来源验证，不能直接开放作者字节码。当前没有运行编译器、恶意包或IL2CPP测试。
