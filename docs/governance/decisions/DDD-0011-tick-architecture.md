...
canon_basis: "SRC-USER-2026-09-05-TICK-ARCHITECTURE；DDD-0010；DDD-0012"
depends_on: ["DDD-0010-host-authority-gameplay-commands.md", "DDD-0012-replication-architecture.md", "../../technical/network-and-persistence.md", "../../technical/architecture-and-performance.md"]
---

# Tick Architecture：固定60Hz权威模拟与多频率更新

## Context / User intent

在 DDD-0010 已锁定 player-hosted authoritative listen server、Gameplay Command Replication、Client Prediction / Server Reconciliation 后，需要确定 Authority Simulation 的时间基线，以及 Network Replication、AI Brain、Physics、Presentation 是否与同一频率绑定。
...
DDD-0011-DEC-A · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
DDD-0011-DEC-B · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
DDD-0011-DEC-C · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE；依赖 DDD-0010 Gameplay Command 模型。
...
这些 packetization 细节仍由后续 Replication/Transport DDD 决定。

核心边界：Gameplay Command 语义与网络 packet 结构解耦。

## Replication rate baseline

DDD-0011-DEC-D · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
DDD-0011-DEC-E · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
DDD-0011-DEC-F · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE；与 DDD-0010 Gameplay/Cosmetic Physics 划分一致。
...
## Projectile networking consequence

DDD-0011-DEC-G · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
## Catch-up / overload behavior

DDD-0011-DEC-H · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
过载时优先削减/延迟：
- low-priority AI Think；
- background simulation；
- low-priority replication；
- cosmetic/presentation work。

不得为了追帧静默丢掉已合法发生的：
- Player movement / action semantics；
- Damage / health transactions；
- Weapon fire；
- Gameplay Physics结果；
...
DDD-0011-DEC-I · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。

Mod API 不允许依赖 Render FPS 产生 Gameplay 结果。禁止把 `OnFrame()` 一类表现帧回调作为伤害、计时、Cooldown或资源规则的Gameplay时间基线。

Mod Gameplay接口优先提供：
- SimulationTime / fixed gameplay time；
...
## Tickrate configurability

DDD-0011-DEC-J · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
DDD-0011-DEC-K · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE。
...
- decision/update cadence；
- replication cadence / event policy；
- priority / relevance；
- overload degradation behavior。

禁止默认继承“60 Hz就全部执行60次”。

## Replication handoff / remaining OPEN

DDD-0012 已完成本DDD原先的下一Gate，正式锁定 Snapshot + Delta State Replication + Reliable Gameplay Events + per-client Interest Management + Dormancy；baseline/dirty-state、latest-state-wins 与 reliable transaction 的语义边界，以及 Join-in-progress / Projectile 的复制方向均见[DDD-0012](DDD-0012-replication-architecture.md)。

因此以下不再作为“Replication架构是否采用”的OPEN问题，但具体实现参数仍OPEN：
...
6. Server Rewind history window和Lag Compensation细节；
7. Projectile prediction/correction实现；
8. Physics correction/smoothing策略；
9. Host Migration snapshot cadence与Authority Epoch恢复协议；
10. 目标带宽预算与Host CPU预算；
11. 具体Networking Provider / Transport。

**下一正式网络 Gate：Lag Compensation / Server Rewind Architecture。**

先裁决Hitscan、Projectile与移动目标在延迟下如何使用Gameplay Command时间参考与Authority历史做命中验证，再继续Host Migration具体协议和Transport Provider选择。
