...
canon_basis: SRC-SSOT-2.0
  §26–§28；SRC-USER-2026-09-05-OFFICIAL-CONTENT-PACKAGES；SRC-USER-2026-09-05-TECH-OPEN-BATCH-AND-LAG-COMP-GATE；SRC-USER-2026-09-05-ASSET-SOURCING-NO-QUOTA
depends_on:
  - 视觉方向与可识别性.md
  - ../技术/模块化产品公开能力与工具链.md
---
...
ASSET-001 · CANON WORKFLOW · 来源：现行[美术ART-006至ART-007](视觉方向与可识别性.md)、SRC-USER-2026-09-05-TECH-OPEN-BATCH-AND-LAG-COMP-GATE与SRC-USER-2026-09-05-ASSET-SOURCING-NO-QUOTA。
...
外部基础资产更适合人类工业设施、管线、梯道、容器、普通机械、基础枪模、Humanoid动画和通用VFX；四名角色关键轮廓、核心武器外壳语言、虚空兽主体、筑路者结构与界桥需要更强原创控制。Visual DNA与模型档位已按所有者要求[重新比较](Meshy工作流研究与美术重选计划.md)，分层壁垒仅为候选；Visual Production Gate通过前仍只做少量代表性搜索、生成、采购和试装，不批量锁死资产库。
...
Schema、hash、状态迁移、禁止发布面和构建路由自动检查执行；条款解释、AI/外包披露、例外和最终发布签署必须人工复核。Workshop包只能包含明确允许该发布面再分发的文件。引用依赖不复制文件，但Steam订阅/依赖关系本身不授予许可；只有上游已允许相应运行时使用、消费者满足其访问条件且引用能力受支持时，才能引用其稳定AssetID。不得用依赖项、自动同步或Cooked格式绕过原文件和示例再分发限制。证据的法定保存期、实际签署人和发布时披露文字取决于当时法域、平台和合同，必须在付费或发布Gate中核对，不伪装成可由GDD永久决定的法律事实。

ASSET-DISTRIBUTION-SURFACES · 2026-09-07受托发布面审查。以下是项目放行规则，不是对尚未选定资产作法律结论。

| 发布面 | 允许前必须核对 | 不可由什么替代 |
|---|---|---|
| 私有制作与本地导入 | 原始文件/修改/协作者访问的实际允许范围 | 购买成功或来源网站仍能打开 |
| 游戏运行包 | 该构建与平台允许嵌入的成品资源及完整来源链 | `Cleared-Internal`、绑定/渲染检查通过 |
| 公共仓库、公开CI制品 | 明确允许公开的原始或派生文件 | 私有CI曾成功打包、文件经过压缩 |
| SDK与Workshop示例 | 明确允许开发者取得并再使用的源文件/派生文件；否则使用原创灰盒示例 | 游戏本体可分发的许可 |
| 玩家模组 | 自有或获准文件，加合法稳定依赖引用 | 已买游戏、Workshop审核或文件hash |

每次构建检查本发布面及全部传递资产来源，不只看根包；拒绝后列出受影响AssetID和制品，不静默漏掉必需资产。采购比例不预定，免费/购买/AI生成均走同一规则；当前没有逐件签署、采购或发布。
