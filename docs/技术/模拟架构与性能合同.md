...
canon_basis: SRC-SSOT-2.0
  §1.5、§22–§23、§29；SRC-USER-2026-09-05-UNITY-ENGINE-LOCK；SRC-USER-2026-09-05-UNITY-URP-GAMEOBJECT-FIRST；SRC-USER-2026-09-05-TICK-ARCHITECTURE
depends_on:
  - Unity与Steam模组技术栈.md
  - 固定步进与多频率模拟.md
---
...
Cardinality/Topology/Capability是数据：官方槽位不渗透Save/Network/UI/Entity。Simulation performance优先；可降VFX/audio voices/远处表示/动画细节，不能暗删合法伤害、已经存在的敌人、投射物/Proc/Summon结果。每类敌人分别声明ThreatCost与ActiveSimulationCost；所有来源在Spawn Commit前由Authority检查跨阵营自主模拟成本和敌对远程/重型硬顶，达到准入上限即不生成新实体；现存敌人合法苏醒不受此门限阻止，最坏连锁负载另行验证。上限只能用实际Profiler与网络证据调整，不能在运行中为救帧偷偷变更。实体由components/profiles/tags组合；AnatomyGraph支持0..N部位、稳定ID、拓扑、命中体、完整度/装甲与增删/再生/转换，能力从图编译供热路径读取。
...
Live Operations只通过受验证的`RunRulesSnapshot`向Operation提供EventRevision、Modifier Bundle、精确内容图和奖励政策；它不进入60 Hz Simulation热路径，也不能下发任意代码。新增武器、敌人和Modifier仍走ContentPackage/Registry，活动只重组已发布能力。控制面、缓存和故障合同见[在线活动控制面](在线活动控制面与版本快照.md)。

普通地点环境也使用同一RunRulesSnapshot，活动身份可空。具体编译、类型化政策、动态阶段、消费者与恢复由[关卡与活动Modifier运行合同](关卡与活动修改器编译及运行合同.md)唯一维护；它通过现有槽位、Stat/Effect、成本/供给与设施状态执行，不另建每日/每周运行时。无限工具与无限端口是公开的资源政策，不是无限数值、无限实体或性能豁免。
...
ARC-006 · CANON · 来源：SRC-USER-2026-09-05-UNITY-ENGINE-LOCK；决策见[Unity引擎与渲染](../治理/选型依据/Unity引擎与渲染.md)。
...
**正式Render Pipeline为Unity 6 URP。** HDRP不建立并行资产或兼容管线。正式材质、Shader、VFX、Lighting、Fog、Decal、LOD和资产采购以URP为唯一生产基线。URP仍需在[重新选择的美术方向](../表现与资产/视觉方向与可识别性.md)代表实机场景中接受GPU frame time、memory、shader variant/build成本、战斗可读性和Steam Deck thermal soak测试；几何档位按样件与实测决定，风格重选不建立第二套渲染管线。

ARC-012 · CANON · 来源：SRC-USER-2026-09-05-TICK-ARCHITECTURE；详见[固定Tick与多频模拟](固定步进与多频率模拟.md)。
...
具体依赖现在由Unity与Steam模组技术栈.md拥有；旧讨论的网络provider/编辑器/脚本选择不再OPEN。普通GameObject-first与局部热路径优化继续，禁止为未来Descent先写通用游戏引擎。数据合同定义稳定ID、字段和状态写入；网络/持久化共用语义但不同投影。所有性能要求仍NOT RUN，显示密度优化不能成为丢合法结果借口。

## 性能证据不能只测正常刷怪波次

ARC-ACTIVATION-PRESSURE · TEST，全部未运行。

除普通有限来源外，代表性压力用例必须包含初始休眠群同时唤醒、分队把不同区域的Active敌人带到同一空间、多个已接管设施与便携工具同时工作、仍在飞行的弹体与多部位破坏，以及主机迁移恢复后的同场负载。参数取批准地图和合法库存，不伪造无来源的无限波次作为日常任务。

分别记录自主单位总成本、敌对职责、玩家/Bot负载、弹体/物理/工具负载及帧时间；来源准入通过不代表这些总成本已被证明适合目标设备。近远动画和视听表示可降级，但不删已存在的合法结果。遇到无法通过的实测负载，先修热路径或缩减批准内容的并发规模；若必须修改所有者的硬需求，再提交范围裁决，不静默降低目标。
