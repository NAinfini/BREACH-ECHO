...
AttackRoot 追踪一次因果攻击；Hit 表示合法命中；DamageEvent/Status/Reaction 由[Build系统](build-algebra.md)解释。视觉后坐、镜头抖动与真实弹道分离。武器成本见[经济](economy-and-support.md)。
...
CMB-002 · CANON · 来源：SRC-SSOT-2.0 §6.3–§6.6。

Melee 不靠必然换血平衡，靠距离/时间/安全风险；Hammer、Knife、Spear、Sword 各承担差异动作。Staff 是可换法术的施法平台，开局必须完整可用，法术奖励与 Relic 分离，可来自 Archive/shrine/cache/专属奖励/Fusion。法术应覆盖 Projectile、Channel、Area、World manipulation、Defense、Summon、Mobility、Control。Energy 以控热换持续，贪爆发付过热或应急 Heat Sink；不能同时永远最高爆发。Heavy/Prototype 使用稀缺资源，对部位、装甲、结构有真实能力，禁止只靠隐藏 CanKillBoss 钥匙。
...
| 参数 | 值与状态 | 来源 |
|---|---|---|
| Staff 初始 Spell | 3 · CANON 官方 profile | SRC-SSOT-2.0 §6.4、§40 |
| Staff 局内目标上限 | 6 · TEST 官方 profile；不是内核上限 | SRC-SSOT-2.0 §6.4、§40 |
| Melee试制集合 | Hammer/Knife/Spear/Sword，共4把 · TEST | SRC-SSOT-2.0 §6.3、§40 |
| 玩家友伤默认 | 0% · CANON；自定义示例10/25/50/100% | SRC-SSOT-2.0 §12.1、§40 |
| 精确DPS/护甲/姿态/热曲线 | OPEN | 待裸武器对照 |

## 示例

CMB-009 · PROPOSED · 来源：本轮系统扩写。
...
CMB-012 · DIRECTION · 来源：SRC-USER-2026-09-04-ENERGY-EM-GUN-FAMILIES。
...
CMB-013 · PROPOSED · 来源：SRC-USER-2026-09-04-ENERGY-EM-GUN-FAMILIES；SRC-USER-2026-09-04-SOL-RECONNECTION-DLC。

Energy Block家族由三把独立武器子型组成，不是一把枪的三模式：单发高伤、连发低伤、强制蓄力。强制蓄力型按蓄力程度形成低蓄力低伤到高蓄力高伤，高蓄力可获得范围爆炸；阈值、每发消耗、Heat、蓄力中断、暴露与友伤风险均OPEN。每把枪只承担自己的角色，不把三种角色塞回每一把枪。

这是对现有Energy家族资源模型的重做方向，与CMB-002“以控热换持续”的CANON基线尚未完成裁决；在用户正式确认覆盖关系前，两者不得被写成已经同时成立的最终规则，也不得据此实现两套长期兼容系统。

未来DLC候选中的Earth能量/电磁武器，在标准Operation必须是有明确成本的sidegrade，不能成为付费战斗力。夸张高火力版本只能属于未来独立ruleset，当前不承诺制作。

## 响应性、取消窗口与玩家技巧

CMB-014 · PROPOSED · 来源：SRC-USER-2026-09-04-RESPONSIVE-GUNPLAY-CANCEL-WINDOWS。
...
切枪或换弹后，ADS与Fire可通过input buffering和合法cancel window尽早衔接，不要求所有恢复帧锁死。蓄力武器必须用高伤、高穿透或高蓄力范围效果等明确回报支付蓄力承诺，不能只是延迟普通一枪；当前射击结算前后可在合法窗口预输入下一轮蓄力，不强迫等待完整发射收尾。缓冲时点、移动/受击/换弹中断均OPEN，且不得突破已声明射速、资源扣除与权威提交顺序。
...
CMB-016 · OPEN · 来源：引用[产品VIS-008](vision.md)。

枪械-only首发仍是高影响条件方向，尚未正式锁定。CMB-002的Staff/Arcane与参数表3/6基线继续有效，直到用户明确裁决；当前不维护永久双轨实现。

## Team Ordnance：世界团队重资产

ORD-001 · DIRECTION · 来源：SRC-USER-2026-09-04-ORDNANCE-MISSIONS。
