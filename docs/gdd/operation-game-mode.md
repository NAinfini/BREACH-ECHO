...
depends_on: ["game-vision-and-design-pillars.md", "../governance/decisions/team-ordnance.md", "../governance/decisions/predator-and-breach-missions.md"]
---
...
唯一首发Operation；入场锁装备身份，局内以WeaponModule/ToolModule/TeamProtocol改变解法。标准长局初值每玩家2保证+1可选机会；安装预览后3s在合法维护点提交，同挂点替换、旧件留世界。原自动Fusion仅Lab/FUTURE，不能吞掉Operation配件。数量和挂点归[测试参数](../production/initial-test-parameters.md)，统一数学归[修改与效果](field-modifications-and-effect-system.md)。

## 玩家旅程与状态

OPS-004 · DECIDED · 来源：本轮模式扩写。
...
| Recon | 缺什么信息，走哪条路 | [世界](facility-systems-and-information-rules.md) | 取得合法位置/设施状态 |
...
| 遭遇级 | 侦察Source→选择打/绕/改环境→执行→进入低压 | 路线安全、资源消耗、世界状态变化 |
| 单局级 | 任务板→配装→Recon→Commit→Execution→Exit→Debrief | 合同结果、详细战报、收藏与横向奖励 |
...
OPS-009 · CANON · 来源：SRC-USER-2026-09-05-LEGACY-ID-LOW-CLEARANCE；权限语义归[设施规则WRD-015](facility-systems-and-information-rules.md)。
...
OPS-013 · DIRECTION · 来源：SRC-USER-2026-09-05-NODES-LORE-ONLY-DESCENT-COMMUNITY-EVENT；世界边界见[世界观NAR-030](canonical-world-history-and-lore.md)。
...
| 现场改装机会与挂点 | 2保证+1可选初值；明确安装替换 | [测试参数](../production/initial-test-parameters.md) |
| 自动Fusion / 无限Relic | Operation禁用；Lab/FUTURE隔离 | [修改与效果](field-modifications-and-effect-system.md) |
| 入场槽位 | 2枪 / 1工具 / 1自由战术模块 | [玩家合同](player-and-input.md) |
| 任务板与难度倍率 | 六Offer与免费局前刷新；倍率仅允许外观收益 | [测试参数](../production/initial-test-parameters.md) |

## 内容接口与边界

OPS-005 · DECIDED · 来源：本轮模式扩写。

首个模板采用[BLACKSTART](../content/blackstart-greybox-mission.md)，主角是Terminal信息与Power Cart，Door/Support作支撑；不额外塞Boss、Predator或谜题大全。所有主线能无Bot单人顺序完成。已完成目标后只关闭该目标可重复奖励来源，不能偷删未取得且设计承诺的资源；世界恶化必须有实际路径与反馈。若能稳定farm，先修奖励来源/可见世界行为，不加隐藏处决表。
