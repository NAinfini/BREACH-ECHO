...
在枪声、四人语音和效果密集时仍能识别危险、队友危机和任务变化。语音网络与频道规则归[合作](../玩法/合作单人机器人与公共匹配.md)。
...
现场协作信息依[UX-FIELD-FEEDBACK](界面信息与可访问性.md)区分危机、操作状态和主动请求。CriticalCue不等人物说完，重复查询/预览/收到不逐条叠确认音；不同真实危机保留可区分的线索。只有合法可听/可知方向才能用于空间声或方向字幕，不从隐藏坐标增加精度。Ping提示音、界面确认音和玩家语音不自行生成AI听觉刺激；真实门、工具、终端操作仍按原声学合同发声，静音或合并音效不改变潜行后果。对白长度、候选去重与抢占仍归叙事调度，不以HUD通知时长裁剪录音。
