...
depends_on: ["architecture-and-performance.md", "gameplay-data-command-and-save-contracts.md", "network-and-persistence.md", "../governance/decisions/agent-first-modding-runtime.md", "../governance/decisions/steam-workshop-mod-runtime.md"]
---
...
多人加入时，Client不手工寻找或修改Host所需Mods。Host在正式Gameplay连接前提供本局精确Package Lock；Client自动比对本地包，对缺失或不匹配的可自动获取包执行下载、安装、验证、staging和profile激活，全部hash一致后才进入Run。Data/Graph/受支持Luau Sandbox Script可走标准同步；社区Managed/Native DLL、可执行文件与动态库由Validator拒绝，不进入授权弹窗或自动同步路径。

MOD-016 · CANON · 来源：SRC-USER-2026-09-05-STEAM-WORKSHOP-PRIMARY；SRC-USER-2026-09-05-STEAM-ONLY-SALES-MODS-DECOUPLED；SRC-USER-2026-09-05-STEAM-ONLY-NETWORK-STACK-APPROVED。

**初始版本正式公开Mod生态只使用Steam Workshop。** Workshop负责发布、发现、存储、订阅、安装和Host缺包自动下载；不实现mod.io、Epic/GOG Mod平台、Host临时传包、官方非Steam CAS或另一套公开Mod Marketplace。游戏自带BREACH Mod Loader/同步协调层，第三方Loader/Manager不是正常玩家使用官方支持Mods的前提。
...
MOD-018 · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST；完整决定见[Steam Workshop Mod Runtime](../governance/decisions/steam-workshop-mod-runtime.md)。

Steam Workshop负责发现、搜索、评分、评论、订阅、发布与下载；游戏内Manager负责Profile、Modpack引用、依赖闭包、冲突、能力、精确hash、多人同步、缓存与诊断。游戏内不重做缩水版Steam商店。玩家可一键恢复Official-only Profile；加入Host产生临时Join Profile，不污染常用Profile。

MOD-019 · CANON/DIRECTION + TEST · 来源：SRC-USER-2026-09-05-TECH-OPEN-BATCH-AND-LAG-COMP-GATE；SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST。

原四个Gate的架构原则已经获批；表中验证项仍不能当作已实现：

| Gate | 推荐裁决 | 仍需验证 |
|---|---|---|
| Mod Manager | Steam做社区发现；游戏内做Profile/Lock/冲突/同步/诊断 | 控制器、错误理解、50包压力与最终视觉 |
| Sandbox Script | Luau Core Compiler + VM；Authority-only Gameplay Script；不使用Lute | Unity 6/IL2CPP/Deck绑定、安全、调试与预算Spike |
| Native/Managed DLL | 初始Workshop与`Mods/Dev`标准包一律拒绝 | 未来若真需要，另立OS/进程隔离威胁模型 |
| 旧exact hash | 已验证本地不可变缓存；Steam当前item校验；拿不到就拒绝Join | 缓存容量、Steam离线/删除/私有化与恢复UX |

Steam的Game Branch Versioning只能帮助游戏分支与Mod兼容，不是任意`content_hash`历史仓库。合作PVE“不做竞技反作弊”也不能推出“Native DLL没有安全问题”：前者接受房主修改玩法，后者会把文件、进程、凭据与整台电脑暴露给不可信代码。

MOD-020 · CANON · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST。

`Profile`是玩家可编辑的顶层选择；应用后编译出新的revision。`Modpack`是可发布的Workshop引用配方，不重新打包其他作者文件。`Package Lock`是包含完整传递依赖、精确hash、schema、runtime/compiler版本、能力和加载序的不可变闭包；Active/Suspended Run固定该Lock。更新只能进入新staging revision，不能改写旧Run。

MOD-021 · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST；用户授权助手完成语言选择；Luau官方Sandbox资料。

Sandbox Script锁定为 **Luau Core Compiler + VM**。不使用提供文件、网络请求和socket等通用能力的Lute，不并行维护MoonSharp、标准Lua、Wasm或C#脚本运行时。Luau Package使用独立environment、只读共享库、内存类别与interrupt；只暴露项目拥有的Capability Handle及验证过的Command。文件、进程、socket、反射、CLR/Unity对象、平台账号、真实墙钟和环境变量不可达。Gameplay Script只由Authority执行；Client Presentation Script无玩法写权限。持久状态只能进入有schema与配额的Package Save Namespace；Hot Reload只用于SDK测试。

MOD-022 · CANON · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST；Microsoft未知代码安全边界资料。

初始版本的标准ContentPackage拒绝社区`*.dll`、原生库、动态Assembly与可执行文件；Workshop自动同步永不执行这些内容。官方随游戏发布的Luau VM或Steam适配器是游戏二进制，不是Mod能力。未来若改变该规则，必须先证明OS/进程级隔离；一个警告框或“仅私人房”不构成安全边界。

MOD-023 · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST；Steamworks Workshop资料。

所有验证通过的包从Workshop安装目录复制进BREACH不可变artifact cache，以`package_id + content_hash`索引。Active/Suspended Run和用户保存Profile pin其闭包；被pin内容不得静默驱逐。Join优先精确本地hash，否则用Steam下载当前item后再校验；不匹配、删除、私有化或不可得则明确拒绝。已有缓存成员可继续，缺缓存的新成员不能加入；Host直传、非Steam公共CAS和静默升级继续禁止。

MOD-024 · TEST · 来源：本轮Mod Runtime整批评审。

Luau Spike先测：全部Gameplay Mod Script总CPU不超过Authority Simulation CPU预算10%；每VM 32 MiB、全体Script 256 MiB；无限循环在当前tick内可interrupt。以上仅为试验起点。还必须覆盖宿主API一次性Spawn洪水、事件/日志/Timer/Save配额、Unity 6 IL2CPP、Windows/Proton、Host Migration、Replay与可定位的源码报告；不能只限制虚拟机指令数就宣布安全。

MOD-025 · CANON/DIRECTION · 来源：SRC-USER-2026-09-05-MOD-RUNTIME-APPROVED-SECURITY-FIRST；SRC-USER-2026-09-05-MOD-CONTENT-CAPABILITY-CLARIFICATION；项目既有公开Mod与可替换官方内容意图。

标准Mod系统必须能组合Data、Graph、Cooked Assets与Luau创建新武器、工具、敌人、模块化地图/房间、任务和Modded Profile角色；规则集与Total Conversion保留在同一Package/Capability架构内。Luau只承担安全逻辑，不替代资产Cooker或空间Validator。初始公开SDK优先完成武器、敌人、任务和模块化空间；角色工具随后，完整TC工具不作为首发Gate。自定义Unity Scene、MonoBehaviour、Editor脚本和未知序列化对象不能绕过Cooker进入运行时。完整能力矩阵见[Steam Workshop Mod Runtime](../governance/decisions/steam-workshop-mod-runtime.md)。
...
MOD-005 · PROPOSED · 来源：本轮用户武器配件底层统一意图；规则责任见[Build](../gdd/field-modifications-and-effect-system.md)。
...
MOD-012 · DIRECTION · 来源：SRC-USER-2026-09-05-UNITY-TECH-DIRECTION；许可与来源责任见[Asset Policy](../production/asset-policy-and-provenance.md)。
