...
| MissionBoard | 全服同轮8个普通Offer、轮换倒计时；活动存在时另列特殊行动及截止时间；每卡显示区域/设施、主/支线、交战合同、Modifier、装备限制、警告、奖励、长度、难度；精确选择或显式多选快速加入 | 选定一个普通或活动Offer/难度，或一组玩家明确接受的Offer/难度；后续轮换或活动到期不中断已锁Run |
| SessionSearchOrCreate | 按精确选择或可接受集合显示现有公开房，或创建Solo/好友/私密/公开房 | 进入一个任务身份、版本与隐私明确的Session；最终锁定前显示实际命中项，无结果时不暗改筛选 |
| Lobby | 已锁任务完整卡、隐私、成员、语音/静音、Ping、包锁、邀请/踢出权限与进入配装入口 | Seat与依赖合法；所有成员都能复查任务后进入配装/Ready |
| Loadout | 角色Seat、两枪/专用近战/工具/模块、队友配置、冲突、资源与详情比较 | 每名真人配装合法；返回Lobby不丢已提交合法选择 |
| ReadyCheck | 最终任务卡、难度、成员、Seat、配装、包锁和未Ready原因 | 全部真人明确Ready且依赖齐全；任一承重变化使相关成员退出Ready |
...
标准创建流程为在MissionBoard选择Offer与难度→选择Solo/好友/私密/公开→创建Session并锁定Mission→分配Seat/验证包锁→进入Lobby→协同配装→Ready。标准公开加入有两种明确入口：精确选择一份Offer/难度；或玩家主动勾选一组可接受的Offer/难度后快速加入。搜索只匹配该明确集合与兼容规则，显示实际任务、地区、延迟估计、任务阶段、人数、是否Modded及下载量→确认→身份/版本/包锁验证→下载与Staging→分配Seat→进入Lobby或中途加入快照。系统不能为缩短等待暗加未选择的难度、任务、地区或Mod状态。Steam邀请和已存在好友小队可以直接进入房间，但加入者必须先看到完整已锁任务卡、难度、规则和包状态，不能被直接推进Ready。
